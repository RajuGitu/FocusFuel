...
function FocusSessions() {
  const { setTimer, setBreakTimer } = useTimerCart();
  const navigate = useNavigate();
  const handleClick = (e) => {
    e.preventDefault();
    const span = e.currentTarget.querySelector('span');
    if (span) {
      const text = span.textContent.trim(); // e.g. "50/10 min"
      const [focusText, breakText] = text.split('/');

      const focusMinutes = parseInt(focusText); // "50" -> 50
      const breakMinutes = parseInt(breakText); // "10 min" -> 10

      setTimer(focusMinutes);       // pass only the number
      setBreakTimer(breakMinutes);  // pass only the number
    }
    navigate('/FocusFuel/study');
  }
    ...
    <div>
      <section className='mb-8'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" >
          <div onClick={handleClick} className='transition-transform opacity duration-200 ease-out transform scale-100 opacity-100 hover:scale-105 md:text-4xl font-extrabold font-display text-2xl font-sans text-white px-4 py-2 rounded-3xl bg-gradient-to-r from-[rgb(113_81_211)] to-[rgb(123,90,222)] '>
            <div className='relative z-10 flex flex-col h-full p-6'>
              ...
              <div className='mt-auto'>
                <button className='transition-colors cursor-pointer backdrop-blur-sm ring-offset-background text-primary-foreground font-medium text-sm py-2 px-4 bg-white/20 rounded-md whitespace-nowrap gap-2 justify-center items-center h-10 inline-flex '>
                  Start Session
